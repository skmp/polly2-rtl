...
// back (1-cyc registered read), verify {start,id,rep,invw[0:3],at,inv[3:0]} per slot.
module dense_span_buffer_selftest;
    reg clk=0; always #5 clk=~clk;
    reg          we; reg [9:0] waddr, w_start, w_id; reg [2:0] w_rep;
    reg  [30:0]  w_invw [0:3]; reg w_at; reg [3:0] w_inv;
    reg  [9:0]   raddr;
    wire [9:0]   r_start, r_id; wire [2:0] r_rep; wire [30:0] r_invw [0:3]; wire r_at; wire [3:0] r_inv;

    dense_span_buffer #(.DEPTH(1024)) dut (
        .clk(clk), .we(we), .waddr(waddr), .w_start(w_start), .w_id(w_id), .w_wm(13'd0), .r_wm(), .w_rep(w_rep),
        .w_invw(w_invw), .w_at(w_at), .w_inv(w_inv),
        .raddr(raddr), .r_start(r_start), .r_id(r_id), .r_rep(r_rep), .r_invw(r_invw),
        .r_at(r_at), .r_inv(r_inv));

    integer errs=0, q;
    task wr(input [9:0] slot, input [9:0] st, input [9:0] id, input [2:0] rep,
            input [30:0] iv0, input at);
        begin
            we=1; waddr=slot; w_start=st; w_id=id; w_rep=rep; w_at=at;
            w_inv=slot[3:0];   // modvol in-volume mask: distinct per slot
    ...
    task chk(input [9:0] slot, input [9:0] st, input [9:0] id, input [2:0] rep,
        ...
        begin
            ...
            if(r_inv!==slot[3:0]) begin $display("slot%0d inv %b!=%b",slot,r_inv,slot[3:0]); errs=errs+1; end
            for(k=0;k<4;k=k+1) if(r_invw[k]!==iv0+k[30:0]) begin $display("slot%0d invw%0d %08x!=%08x",slot,k,r_invw[k],iv0+k[30:0]); errs=errs+1; end
        end
    endtask

    initial begin
        we=0; raddr=0; w_start=0; w_id=0; w_rep=0; w_at=0; w_inv=0; for(q=0;q<4;q=q+1) w_invw[q]=0;
        repeat(2) @(posedge clk);
        // dense slots 0,1,2 with distinct spans
        wr(10'd0, 10'd  0, 10'd100, 3'd4, 31'h2AAA_0000, 1'b1);
        wr(10'd1, 10'd 37, 10'd101, 3'd1, 31'h3BBB_0000, 1'b0);
        wr(10'd2, 10'd512, 10'd102, 3'd2, 31'h4CCC_0000, 1'b1);
        wr(10'd3, 10'd1020,10'd103, 3'd3, 31'h5DDD_0000, 1'b0);
        @(posedge clk);
        chk(10'd0, 10'd  0, 10'd100, 3'd4, 31'h2AAA_0000, 1'b1);
        chk(10'd1, 10'd 37, 10'd101, 3'd1, 31'h3BBB_0000, 1'b0);
        chk(10'd2, 10'd512, 10'd102, 3'd2, 31'h4CCC_0000, 1'b1);
        chk(10'd3, 10'd1020,10'd103, 3'd3, 31'h5DDD_0000, 1'b0);

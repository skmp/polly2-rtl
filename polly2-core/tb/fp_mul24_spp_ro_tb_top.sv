// Bit-exactness harness for fp_mul24_spp_ro vs the combinational fp_mul24.
...
module fp_mul24_spp_ro_tb_top (
    ...
    fp_mul24_spp_ro dut (.clk(clk), .reset(reset), .stall(1'b0), .in_valid(in_valid),
                         .a(a), .b(b), .out_valid(out_valid), .y(y));
    fp_mul24 u_ref (.a(a), .b(b), .y(y_ref));
endmodule

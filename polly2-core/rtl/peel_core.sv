...
module peel_core import tsp_pkg::*; #(
    ...
    // DDR_OUT only sizes the arbiter's BOOKKEEPING. It can never create outstanding
    // reads: the backend gates acceptance with ddr_resp.busy, so this FIFO just has to be
    // deep enough not to be the limiter itself. Avalon-MM allows as many pending reads as
    // the SLAVE advertises (maximumPendingReadTransactions) and returns data strictly in
    // issue order, which is the property the order FIFO relies on; the HPS f2h SDRAM
    // port's actual window lives in the generated Qsys component, not in this repo.
    // NOTE the on-hardware master in quartus/peel_core_top is still SINGLE-outstanding
    // (rd_issue = ddr_req.rd && !rd_inflight), so on the DE10 the per-client caps below
    // are moot until that master is pipelined too.
    ...
    wire [OFW-1:0] of_head = of_rp[OFW-1:0];
    integer di;
...
`endif
    ...
    reg                    tvw_pbc_v;           // u_taginvw walk write (clear/B-image)
    reg  [CHUNK_AW-1:0]    tvw_pbc_a;
    reg                    tvw_pbc_bmode;       // 1 = slot-B materialize data
    reg                    pb_zkeep;            // pb write = z_keep depth-restore (not peel swap)
    reg                    pb_bfin;             // pb write = final-pass B fold (BFIN walk)
    wire [RAS_LANES-1:0]   b_pass_lp;           // per-lane slot-A accept (for dt_pt)
    wire [RAS_LANES-1:0]   b_pass_b;            // per-lane slot-B accept (2-layer)
    wire [RAS_LANES-1:0]   b_bstg;              // per-lane slot-B STAGED (accept or slide)
    wire [RAS_LANES-1:0]   b_disp;              // per-lane displaced-out (demote b_oldtag)
    // slot-B image export from the PeelBuffers walk (u_peel -> u_taginvw materialize)
    wire [RAS_LANES-1:0]    pb_bt_valid, pb_bt_pt;
    wire [32*RAS_LANES-1:0] pb_bt_tag;
    wire [31*RAS_LANES-1:0] pb_bt_invw;
    wire                    pb_bt_any;
...
`endif
    ...
    // two-layer peel kill switch: +onelayer (sim-only bisect aid) restores the
    // exact single-slot peel loop (slot B never stages, no B hands, no
    // CLR2/BFIN). Constant ON in synthesis.
    reg tl2_cfg;
`ifndef SYNTHESIS
...
`endif
    wire tl2_en = tl2_cfg;   // always ON in synthesis; +onelayer is the sim bisect
    peel_tile_buffer #(.LANES(RAS_LANES)) u_peel (
        ...
        .b_2l(tl2_en), .b_ispt(b_which),
        .b_pass_lp(b_pass_lp), .b_pass_b(b_pass_b), .b_bstg(b_bstg), .b_more(b_more),
        .b_disp(b_disp), .b_oldtag(b_oldtag),
        .b_we(b_we),
    ...
    reg  [31:0] fc_tag;
    reg         fc_cfg;      // +nofrontcull (sim-only bisect aid) kill switch
`ifndef SYNTHESIS
    initial fc_cfg = !$test$plusargs("nofrontcull");
`else
    initial fc_cfg = 1'b1;
`endif
    wire fc_en = fc_cfg;     // always ON in synthesis; +nofrontcull is the sim bisect
      ...
      for (gsc = 0; gsc < RAS_LANES; gsc = gsc + 1) begin : scwt
        if (gsc == 0) begin : scwt0
          assign sc_wr_tag[0 +: 32] = fc_go ? fc_tag
                                            : (b_disp[0] ? b_oldtag[0 +: 32] : b_tag);
        end else begin : scwtn
          assign sc_wr_tag[32*gsc +: 32] = b_disp[gsc] ? b_oldtag[32*gsc +: 32] : b_tag;
        end
      end
    endgenerate
    ...
    // Time-0 procedural, not elaboration-time inside a generate: Quartus rejects
    // SystemVerilog elaboration system tasks there (10170).
`ifndef SYNTHESIS
    initial
      if ((TI_COPIES < 2) || (TI_COPIES & (TI_COPIES - 1)))
          $error("peel_core: TI_COPIES must be a power of two >= 2 (got %0d)", TI_COPIES);
`endif
    ...
    taginvw_tile_buffer #(.LANES(RAS_LANES), .COPIES(TI_COPIES)) u_taginvw (
        ...
        .clr_depth(regs.isp_backgnd_d[30:0]), .clr_tag(regs.isp_backgnd_t),
        // PeelBuffers walk write: valid-clear, or (two-layer peel) the slot-B
        // image MATERIALIZE - the walk's B read-back goes straight into the copy
        // the B layer will shade from. tvw_pbc_* are driven beside pb_buf* below.
        .pbc_valid(tvw_pbc_v), .pbc_addr(tvw_pbc_a),
        .pbc_bmode(tvw_pbc_bmode),
        .pbc_bv(pb_bt_valid), .pbc_btag(pb_bt_tag),
        .pbc_binvw(pb_bt_invw), .pbc_bpt(pb_bt_pt),
    ...
    generate
      for (gcol = 0; gcol < 2; gcol = gcol + 1) begin : gcolbuf
        ...
        color_tile_buffer #(.DEPTH(TILE_W*TILE_H)) u_col (
            ...
            .bl_cb_valid(is_prod && cb_wr_en), .cb_id(cb2_id), .cb_data(cb_blend_out),
...
`endif
    ...
    // from the spanner; cons_seq_r feeds the consumed watermark back (declared
    // with the reader regs below)
    wire [SPAN_AW:0]   spv_head_live;
    wire [12:0]        spv_sp_wm, spv_setup_seq;
    wire [12:0]        dsr_wm;
    wire [9:0]         dsr_start;
    wire [9:0]         dsr_id;
    wire [2:0]         dsr_rep;
    ...
    spanner_v2 #(.NSLOT(TILE_W*TILE_H), .SLOTW(10),
        ...
        .setup_seq(spv_setup_seq), .cons_seq(cons_seq_r),
...
`endif
    ...
    // This lives at stage CC, not CB: tsp_blend REGISTERS its alpha-test verdict in
    // S1, so bl_at_pass (like cb_blend_out) describes the CB operands one cycle
    // later - hence the cb2_* copies of id/ptres/invw.
    ...
    wire bl_at_pass;    // alpha-test verdict (always 1 when !cb_at_en); valid at CC
    ...
    localparam S_IDLE=0, S_RA=1, S_STATE=2,
               ...
               // TWO-LAYER peel walk extras (see the S_PEEL_BUF_RUN comment):
               S_PEEL_CLR2=30,             // credit wait for the next raster copy
               S_PEEL_CLR2_RUN=31,         // ...and its 128-chunk taginvw clear
               S_PEEL_BFIN=37,             // final pass staged a B: credit wait
               S_PEEL_BFIN_RUN=38,         // ...read-only walk materializing it
    ...
    reg          pass_drew;
    reg          pass_bany;     // this peel pass staged >=1 slot-B fragment
    reg          bany_l;        // ...latched at the pass decision for the walk
    ...
    reg [SPAN_AW:0] span_left; // (unused by the streaming walk; kept for reset sites)
...
`ifndef SYNTHESIS
    ...
    integer pc_peel_b;          // second-layer (slot B) images handed to shade
...
`endif
    ...
    always @(*) begin
        ...
        pb_bufrd_valid = (st == S_PEEL_BUF_RUN) || (st == S_ZK_INV)
                      ...
                      || (st == S_PEEL_BFIN_RUN);   // read-only: B materialize
        pb_bufrd_addr  = pb_rd;
        ...
        pb_bfin        = (st == S_PEEL_BFIN_RUN);     // fold B into "last drawn"
        // (stage-B write is driven by the b_valid port directly on u_peel)

        // ---- u_taginvw walk write (valid-clear / slot-B materialize) ----
        // Follows pb_bufwr for the RMW walks (as before), plus the two-layer
        // extras: the final-pass B materialize (read-only on the peel RAM, so it
        // cannot ride pb_bufwr) and the phase-2 clear of the next raster copy
        // (no peel RAM traffic at all - a bare 128-chunk counter walk).
        tvw_pbc_v = pb_bufwr_valid
                  || ((st == S_PEEL_BFIN_RUN) && pb_pipe)
                  || (st == S_PEEL_CLR2_RUN);
        tvw_pbc_a = pb_i;
        // B data goes in on the peel-walk write (the read-back of the pass just
        // drained) and on the final materialize; everything else is the clear.
        tvw_pbc_bmode = ((st == S_PEEL_BUF_RUN) && !first_peel)
                     || (st == S_PEEL_BFIN_RUN);
...
`ifndef SYNTHESIS
            ...
            pc_sort_skip<=0; pc_front_cull<=0; pc_fcull_decl<=0; pc_peel_b<=0;
...
`endif
            ...
            if (b_valid) begin
                ...
                if (|b_bstg) pass_bany <= 1'b1;    // ...and >=1 second-layer fragment
                                                   // (direct B-accept OR A-slide)
            end
...
`endif
            ...
            // only the id/valid and the PT-resolve context ride here
            cb2_valid <= cb_valid;
            if (cb_valid) begin
                cb2_id    <= cb_id;
                cb2_ptres <= cb_ptres;
            ...
            // ---- PT-resolve feedback (CC stage, the single ordered authority) ----
            ...
            if (cb2_valid && cb2_ptres && !pt_res_bit) begin
                ...
                if (!bl_at_pass) begin
                    ...
                    if (cb2_id[4:0] > pt_fbb_x1) pt_fbb_x1 <= cb2_id[4:0];
                    if (cb2_id[9:5] < pt_fbb_y0) pt_fbb_y0 <= cb2_id[9:5];
                    if (cb2_id[9:5] > pt_fbb_y1) pt_fbb_y1 <= cb2_id[9:5];
                end
            end
...
`endif
                    ...
                    begin
                        ...
                        // two-layer: if this pass staged a B image, THAT hand (the
                        // materialize walk's) closes the tile, not A's
                        ti_last [htile] <= !more_to_draw && !pass_bany && wo_l;
                        ti_ptres[htile] <= 1'b0;
...
`endif
                        if (more_to_draw && peel_pass < PEEL_MAX_PASS[7:0]) begin
                            ...
                            pass_drew <= 1'b0;   // next pass: track its accepts
                            bany_l    <= pass_bany;   // the walk hands this pass's B
                            pass_bany <= 1'b0;
                            if (has_pt) begin
                            ...
                            has_pt <= 1'b0; has_tr <= 1'b0;
                            if (pass_bany) begin
                                // the FINAL pass staged a second layer: materialize
                                // and hand it before acking. Read-only on the peel
                                // RAM - its post-peel state must survive for z_keep
                                // successor entries exactly as today.
                                st <= S_PEEL_BFIN;
                            end else begin
                                ra_ack.list_done <= 1'b1; st <= S_RA_ACK;
                            end
...
`endif
            ...
            S_PEEL_BUF_RUN: begin
                ...
                if (pb_pipe && pb_i == CHUNK_AW'(NCHUNK-1)) begin
                    ...
                    // back-to-front) and go clear the NEXT raster copy; if not,
                    // the all-invalid materialize WAS the clear - proceed as the
                    // single-layer flow did. (first_peel walks write the plain
                    // clear and never hand.)
                    if (!first_peel && bany_l) begin
                        ti_ready[htile] <= 1'b1;
                        ti_mode [htile] <= 1'b1;                 // PEEL (valid-gated)
                        ti_last [htile] <= 1'b0;                 // the loop continues
                        ti_ptres[htile] <= 1'b0;
                        ti_postonly[htile] <= 1'b0;
                        ti_tx[htile] <= cur_tx; ti_ty[htile] <= cur_ty;
                        htile <= htile + 1'b1;
`ifndef SYNTHESIS
                        pc_hand <= pc_hand + 1;
                        pc_peel_b <= pc_peel_b + 1;
`endif
                        st <= S_PEEL_CLR2;
                    end else st <= S_OL_RUN;
                end else if (pb_rd != CHUNK_AW'(NCHUNK-1)) begin
                    pb_rd <= pb_rd + 1'b1;
                end
            end

            // ---- TWO-LAYER walk extras ----
            // phase 2: the B hand consumed htile, so the NEXT raster copy still
            // needs its valid-clear (single-layer got it from the pbc walk write,
            // which phase 1 spent on the B image). Bare 128-chunk counter walk on
            // u_taginvw only; raster stays fenced until it completes.
            S_PEEL_CLR2: if (!ti_ready[htile]) begin
                pb_i <= '0;
                st <= S_PEEL_CLR2_RUN;
            end
            S_PEEL_CLR2_RUN: begin
                if (pb_i == CHUNK_AW'(NCHUNK-1)) st <= S_OL_RUN;
                else pb_i <= pb_i + 1'b1;
            end
            // final-pass B materialize: the peel loop ended with a staged second
            // layer that no following walk would deliver. taginvw gets the B
            // image, and the peel RAM gets the pb_bfin fold (zb <- B when staged:
            // the post-peel "last drawn depth" contract z_keep successors read;
            // every other field is kept verbatim), then hand + ack.
            S_PEEL_BFIN: if (!ti_ready[htile]) begin
                pb_rd <= '0; pb_i <= '0; pb_pipe <= 1'b0;
                st <= S_PEEL_BFIN_RUN;
            end
            S_PEEL_BFIN_RUN: begin
                pb_pipe <= 1'b1;
                pb_i    <= pb_rd;
                if (pb_pipe && pb_i == CHUNK_AW'(NCHUNK-1)) begin
                    ti_ready[htile] <= 1'b1;
                    ti_mode [htile] <= 1'b1;
                    ti_last [htile] <= wo_l;   // the tile's final shade
                    ti_ptres[htile] <= 1'b0;
                    ti_postonly[htile] <= 1'b0;
                    ti_tx[htile] <= cur_tx; ti_ty[htile] <= cur_ty;
                    htile <= htile + 1'b1;
...
`ifndef SYNTHESIS
                ...
                $display("  TWO-LAYER:   %0d second-layer images handed (passes saved ~= that many)",
                    pc_peel_b);
...
`endif
            ...
            // head, count UNKNOWN (md_fin=0). The reader may begin consuming this
            // pass's spans as they are written; G_RUN finalizes the count at the
            // spanner's busy-fall.
...
`endif
            ...
            R_RUN: if (!pp_stall) begin
                ...
                // a slot read resolved on dsr_* this cycle, AND its planes are
                // WRITTEN (streaming setup gate: setup_seq has reached the span's
                // alloc watermark; the subtraction is wrap-safe, distance < 4096)
                dsr_rdy   = sb_rd_pend && !((spv_setup_seq - dsr_wm) >> 12);
                ...
                // cons_seq: the pixel LEAVING s2 this cycle (accepted by pp, since
                // !pp_stall here) was its span's last -> every tsg read of that
                // span has resolved; its planes may age out of the window.
                if (s2_v && s2_last) cons_seq_r <= s2_wm;
...
`endif
                ...
                // dsr_addr = sp_rdp is held stable, so the in-flight slot's registered read
                // stays valid on dsr_* until CONSUMED into the skid (fill). On fill, advance
                // sp_rdp to the next RING slot; its read then needs one cycle to resolve, so
                // sb_rd_pend drops for that one cycle. sp_rdp is a FULL-WIDTH free-running
                // pointer (incl. wrap MSB) so availability is the pointer comparison
                // sp_rdp != span_head_live - never an equality against a wrapped end.
                //
...
`endif
            ...
            RS_IDLE: if (!pq_empty && !(st == S_PEEL_BUF || st == S_PEEL_BUF_RUN
                                        || st == S_PEEL_CLR2 || st == S_PEEL_CLR2_RUN
                                        || st == S_PEEL_BFIN || st == S_PEEL_BFIN_RUN
...
`ifndef SYNTHESIS
    ...
    assign occ_ev_ad[5] = {u_shade.u_tex.u_fetch.u_vq4.ev_fetch_a, 2'b00};
